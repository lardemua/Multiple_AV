...
visualization_msgs::Marker makeEllipse(Point center, double A, double B,
                                       double phi, string ns,
...
class Markers
{
   ...
   *
   * The updated marker will be marked for publishing, this marker may or may
   * not already be present in the class.
   *
   ...
   */
    void update(visualization_msgs::Marker &marker)
    {
        for (uint i = 0; i < markers.size(); ++i)
            if (markers[i].ns == marker.ns &&
                markers[i].id == marker.id) // Marker found
            {
...
class MotionModel : public Kalman::EKFilter<double, 0, false, false, false>
{
  public:
    ...
    void makeBaseW() { setIdentity(W, 4, 1.0); }

    /**
      \brief Make measurement noise sensitivity matrix
      */
    void makeBaseV() { setIdentity(V, 2, 1.0); }
    ...
    /**
      \brief Make measurement, used when measurement is not possible (i'm not
     using it now)
      */
    void makeMeasure()
    {
  ...
  public:
    GNN(ros::NodeHandle nh_) : nh(nh_)
    {
        ...
        if (pos != string::npos)
        {
            ...
            string package = parameters_url.substr(
                pos + package_tag.length(), fpos - (pos + package_tag.length()));
        ...
        markers_publisher =
            nh.advertise<visualization_msgs::MarkerArray>("/targets_markers", 1000);
        measurements_markers_publisher =
            nh.advertise<visualization_msgs::MarkerArray>("/measurements_markers",
                                                          1000);
    ...
    void pointCloudToVector(const sensor_msgs::PointCloud2 &cloud,
    ...
    void clustering(const vector<Point::Ptr> data,
                    ...
                    double clustering_distance)
    {
        ...
        for (uint p = 0; p < data.size(); p++)
        {
            ...
            // Go though all the other points and see which ones associate with this
            // measurement
    ...
    void recursiveAssociation(const vector<Point::Ptr> data,
                              vector<pair<Target::Ptr, bool>> &point_association,
                              double clustering_distance, uint p)
    {
        Target::Ptr null_ptr;
    ...
    void updateSearchAreas(const vector<Target::Ptr> targets)
    {
        for (Target::Ptr target : targets)
        {
            ...
            target->search_area.ellipse_A =
                not_found_factor * pow(not_found_counter, 2) +
                A_innovation_factor * innovation_error + default_size +
                size_factor * size;
            target->search_area.ellipse_B =
                not_found_factor * pow(not_found_counter, 2) +
                B_innovation_factor * innovation_error + default_size +
                size_factor * size;
            ...
            if (target->life_time <
                5) /// Small bonus to new objects @todo I should use a equation that
                   /// allowed a more soft transition
            {
    ...
    void associateTargets(const vector<Target::Ptr> current_targets,
                          vector<Target::Ptr> &targets)
    {
        ...
        for (Target::Ptr target : targets)
        {
            ...
            for (Target::Ptr current_target :
                 current_targets) /// Run thorough all the new objects
            {
            ...
            if (association_found)
            {
                ...
                for (Target::Ptr existing_target : targets)
                {
                    ...
                    dist = associated_new_target->centroid.euclideanDistance(
                        existing_target->predicted_position);
                ...
                // Exclusion zone B
                if (min_dist < ezB &&
                    closest_existing_target->life_time > target->life_time)
                    association_found = false;
            }

            if (association_found &&
                associated_new_target->found == false) /// New target found in list
            {
        ...
        targets.erase(remove_if(targets.begin(), targets.end(),
                                [](Target::Ptr t) { return t->to_remove; }),
                      targets.end());
        ...
        for (Target::Ptr current_target : current_targets)
        {
            ...
            for (Target::Ptr target : targets)
            {
                dist_to_object = current_target->estimated_position.euclideanDistance(
                    target->measurement);
    ...
    void createOutputTargets(const vector<Target::Ptr> targets,
                             mtt::TargetList &target_list)
    {
        for (Target::Ptr target : targets)
        {
            ...
            out_target.pose.orientation =
                tf::createQuaternionMsgFromYaw(target->estimated_velocity.angle());
    ...
    createTargetMarkers(const vector<Target::Ptr> targets)
    {
        ...
        for (int i = 0; i < targets.size(); i++)
        {
            ...
            if (velocity > 0.1)
            {
                ...
                marker_velocity.points[1].x =
                    target->estimated_position.x +
                    target->estimated_velocity.module() *
                        cos(target->estimated_velocity.angle()) * velocity_scale;
                marker_velocity.points[1].y =
                    target->estimated_position.y +
                    target->estimated_velocity.module() *
                        sin(target->estimated_velocity.angle()) * velocity_scale;
            ...
            //                 visualization_msgs::Marker ellipse =
            //                 makeEllipse(target->predicted_position,target->search_area.ellipse_A,target->search_area.ellipse_B,target->search_area.angle,"search
            //                 areas",colormap.color(target->id),target->id);
            visualization_msgs::Marker ellipse = makeEllipse(
                target->predicted_position, 0.5, 0.4, target->search_area.angle,
                "simple_marker", colormap.color(target->id), target->id);
    ...
    void hungarianMatching(vector<Target::Ptr> &targets,
                           vector<Target::Ptr> &measurements)
    {
        ...
        for (uint i = 0; i < assignments.size(); i++)
        {
            ...
            for (Target::Ptr current_target : targets)
            {
                ...
                double dist = target->centroid.euclideanDistance(
                    current_target->predicted_position);
        ...
        for (Target::Ptr measurement : measurements)
        {
            ...
            for (Target::Ptr target : targets)
            {
                dist_to_object = measurement->estimated_position.euclideanDistance(
                    target->measurement);
