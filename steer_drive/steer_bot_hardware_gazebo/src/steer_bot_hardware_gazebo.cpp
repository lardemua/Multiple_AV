...
SteerBotHardwareGazebo::SteerBotHardwareGazebo()
    : gazebo_ros_control::RobotHWSim(), ns_("steer_bot_hardware_gazebo/"),
      log_cnt_(0) {}

bool SteerBotHardwareGazebo::initSim(
    const std::string &robot_namespace, ros::NodeHandle nh,
  ...
  ROS_INFO_STREAM("enable_ackermann_link = "
                  << (enable_ackermann_link_ ? "true" : "false"));

  // Position joint limits interface
  std::vector<std::string> cmd_handle_names =
      front_steer_jnt_pos_cmd_interface_.getNames();
  for (size_t i = 0; i < cmd_handle_names.size(); ++i) {
    ...
    if (name != virtual_front_steer_jnt_names_[INDEX_RIGHT] &&
        name != virtual_front_steer_jnt_names_[INDEX_LEFT])
      continue;
  ...
  for (size_t i = 0; i < virtual_jnt_cnt_; ++i) {
    ...
    ROS_INFO_STREAM("Trying to set pid param of '"
                    << jnt_name << " ' at PID proc in init()");
    if (!pids_[i].init(joint_nh)) {
      ROS_INFO_STREAM("Faied to set pid param of '"
                      << jnt_name << " ' at PID proc in init()");
      return false;
    }
    ROS_INFO_STREAM("Succeeded to set pid param of '"
                    << jnt_name << " ' at PID proc in init()");
  }
...
void SteerBotHardwareGazebo::readSim(ros::Time time, ros::Duration period) {
  for (int i = 0; i < sim_joints_.size(); i++) {
    ...
    if (gazebo_jnt_name == virtual_rear_wheel_jnt_names_[INDEX_RIGHT]) {
      GetCurrentState(virtual_rear_wheel_jnt_pos_, virtual_rear_wheel_jnt_vel_,
                      virtual_rear_wheel_jnt_eff_, INDEX_RIGHT, i);
    } else if (gazebo_jnt_name == virtual_rear_wheel_jnt_names_[INDEX_LEFT]) {
      GetCurrentState(virtual_rear_wheel_jnt_pos_, virtual_rear_wheel_jnt_vel_,
                      virtual_rear_wheel_jnt_eff_, INDEX_LEFT, i);
    } else if (gazebo_jnt_name == virtual_front_wheel_jnt_names_[INDEX_RIGHT]) {
      GetCurrentState(virtual_front_wheel_jnt_pos_,
                      virtual_front_wheel_jnt_vel_,
                      virtual_front_wheel_jnt_eff_, INDEX_RIGHT, i);
    } else if (gazebo_jnt_name == virtual_front_wheel_jnt_names_[INDEX_LEFT]) {
      GetCurrentState(virtual_front_wheel_jnt_pos_,
                      virtual_front_wheel_jnt_vel_,
                      virtual_front_wheel_jnt_eff_, INDEX_LEFT, i);
    } else if (gazebo_jnt_name == virtual_front_steer_jnt_names_[INDEX_RIGHT]) {
      GetCurrentState(virtual_front_steer_jnt_pos_,
                      virtual_front_steer_jnt_vel_,
                      virtual_front_steer_jnt_eff_, INDEX_RIGHT, i);
    } else if (gazebo_jnt_name == virtual_front_steer_jnt_names_[INDEX_LEFT]) {
      GetCurrentState(virtual_front_steer_jnt_pos_,
                      virtual_front_steer_jnt_vel_,
                      virtual_front_steer_jnt_eff_, INDEX_LEFT, i);
    } else {
      // do nothing
    }
  }

  front_steer_jnt_pos_ = (virtual_front_steer_jnt_pos_[INDEX_RIGHT] +
                          virtual_front_steer_jnt_pos_[INDEX_LEFT]) /
                         virtual_front_steer_jnt_pos_.size();
  front_steer_jnt_vel_ = (virtual_front_steer_jnt_vel_[INDEX_RIGHT] +
                          virtual_front_steer_jnt_vel_[INDEX_LEFT]) /
                         virtual_front_steer_jnt_vel_.size();
  front_steer_jnt_eff_ = (virtual_front_steer_jnt_eff_[INDEX_RIGHT] +
                          virtual_front_steer_jnt_eff_[INDEX_LEFT]) /
                         virtual_front_steer_jnt_eff_.size();
}
...
void SteerBotHardwareGazebo::writeSim(ros::Time time, ros::Duration period) {
  ...
  for (int i = 0; i < sim_joints_.size(); i++) {
    ...
    // wheels
    if (gazebo_jnt_name == virtual_rear_wheel_jnt_names_[INDEX_RIGHT]) {
      // const double eff_cmd = 2*ComputeEffCommandFromVelError(INDEX_RIGHT,
      // REAR, period); sim_joints_[i]->SetForce(0u, eff_cmd); if(log_cnt_ % 500
      // == 0)
      //{
      //  ROS_DEBUG_STREAM("rear_wheel_jnt_vel_cmd_ = " <<
      //  rear_wheel_jnt_vel_cmd_);
      //  ROS_DEBUG_STREAM("virtual_rear_wheel_jnt_vel_[INDEX_RIGHT] = " <<
      //  virtual_rear_wheel_jnt_vel_[INDEX_RIGHT]);
      //  ROS_DEBUG_STREAM("error[INDEX_RIGHT] " <<  rear_wheel_jnt_vel_cmd_ -
      //  virtual_rear_wheel_jnt_vel_[INDEX_RIGHT]);
      //  ROS_DEBUG_STREAM("command[INDEX_RIGHT] = " << eff_cmd);
      //}
      sim_joints_[i]->SetVelocity(0, rear_wheel_jnt_vel_cmd_);
    } else if (gazebo_jnt_name == virtual_rear_wheel_jnt_names_[INDEX_LEFT]) {
      // const double eff_cmd = 2*ComputeEffCommandFromVelError(INDEX_LEFT,
      // REAR, period); sim_joints_[i]->SetForce(0u, eff_cmd); if(log_cnt_ % 500
      // == 0)
      //{
      //  ROS_DEBUG_STREAM("rear_wheel_jnt_vel_cmd_ = " <<
      //  rear_wheel_jnt_vel_cmd_);
      //  ROS_DEBUG_STREAM("virtual_rear_wheel_jnt_vel_[INDEX_LEFT] = " <<
      //  virtual_rear_wheel_jnt_vel_[INDEX_LEFT]);
      //  ROS_DEBUG_STREAM("error[INDEX_LEFT] " <<  rear_wheel_jnt_vel_cmd_ -
      //  virtual_rear_wheel_jnt_vel_[INDEX_LEFT]);
      ...
      // const double eff_cmd = 2*ComputeEffCommandFromVelError(INDEX_RIGHT,
      // FRONT, period); sim_joints_[i]->SetForce(0u, eff_cmd);
      // ROS_INFO_STREAM("command[INDEX_RIGHT] = " << eff_cmd);
      sim_joints_[i]->SetVelocity(
          0, rear_wheel_jnt_vel_cmd_); // added for gazebo in kinetic
    } else if (gazebo_jnt_name == virtual_front_wheel_jnt_names_[INDEX_LEFT]) {
      // const double eff_cmd = 2*ComputeEffCommandFromVelError(INDEX_LEFT,
      // FRONT, period); sim_joints_[i]->SetForce(0u, eff_cmd);
    ...
    // steers
      ...
      ROS_INFO_STREAM("front_steer_jnt_pos_ '" << front_steer_jnt_pos_
                                               << " ' at writeSim()");
    } else if (gazebo_jnt_name == virtual_front_steer_jnt_names_[INDEX_RIGHT]) {
      double pos_cmd = 0.0;
      if (enable_ackermann_link_) {
        ...
        pos_cmd = atan2(2 * h * tan(front_steer_jnt_pos_cmd_),
  ...
  front_steer_jnt_pos_ = front_steer_jnt_pos_cmd_;
  if (log_cnt_ % 500 == 0) {
    ROS_DEBUG_STREAM("front_steer_jnt_pos_ '" << front_steer_jnt_pos_
                                              << " ' at writeSim()");
  }
  // wheel joint for steer_drive_controller
  rear_wheel_jnt_pos_ = 0.5 * (virtual_rear_wheel_jnt_pos_[INDEX_RIGHT] +
                               virtual_rear_wheel_jnt_pos_[INDEX_LEFT]);
  rear_wheel_jnt_vel_ = 0.5 * (virtual_rear_wheel_jnt_vel_[INDEX_RIGHT] +
                               virtual_rear_wheel_jnt_vel_[INDEX_LEFT]);
  if (log_cnt_ % 500 == 0) {
    ROS_DEBUG_STREAM("rear_wheel_jnt_pos_ '" << rear_wheel_jnt_pos_
                                             << " ' at writeSim()");
    ROS_DEBUG_STREAM("rear_wheel_jnt_vel_ '" << rear_wheel_jnt_vel_
                                             << " ' at writeSim()");
  }
}
...
void SteerBotHardwareGazebo::RegisterWheelInterface() {
  ...
  for (int i = 0; i < virtual_front_wheel_jnt_names_.size(); i++) {
    this->RegisterInterfaceHandles(
        ...
        virtual_front_wheel_jnt_vel_[i],
        virtual_front_wheel_jnt_eff_
            [i] /*, virtual_front_wheel_jnt_vel_cmd_[i]*/);
  }
}
...
double SteerBotHardwareGazebo::ComputeEffCommandFromVelError(
  ...
  const double effort = clamp(command, -effort_limit, effort_limit);
  return effort;
}

void SteerBotHardwareGazebo::GetCurrentState(std::vector<double> &_jnt_pos,
                                             std::vector<double> &_jnt_vel,
                                             std::vector<double> &_jnt_eff,
                                             const int _if_index,
                                             const int _sim_jnt_index) {
#if GAZEBO_MAJOR_VERSION >= 9
  _jnt_pos[_if_index] += angles::shortest_angular_distance(
      _jnt_pos[_if_index], sim_joints_[_sim_jnt_index]->Position(0u));
#else
  _jnt_pos[_if_index] += angles::shortest_angular_distance(
      _jnt_pos[_if_index], sim_joints_[_sim_jnt_index]->GetAngle(0u).Radian());
#endif
...
PLUGINLIB_EXPORT_CLASS(steer_bot_hardware_gazebo::SteerBotHardwareGazebo,
